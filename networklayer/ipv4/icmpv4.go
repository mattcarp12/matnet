...
func (icmp *ICMPv4Header) Marshal() ([]byte, error) {
...
func (icmp *ICMPv4) EchoReply(skb *netstack.SkBuff, requestHeader *ICMPv4Header) {
	...
	rawHeader, _ := icmpHeader.Marshal()
	icmpHeader.Checksum = Checksum(rawHeader)

	// Create new skb for ICMP echo reply
	rawReply, _ := icmpHeader.Marshal()
	...
	sk := netstack.NewSocket()
	sk.Address = netstack.SocketAddress{
		DestIP: skb.GetL3Header().GetSrcIP(),
	}
